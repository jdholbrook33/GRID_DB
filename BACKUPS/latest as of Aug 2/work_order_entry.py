...
def work_order_entry():
    logger.debug("Entering work_order_entry function")
            ...
            if (input) {
                ...
                input.addEventListener('input', function(e) {
                    ...
                    if (this.value.length === 8) {
                        console.log('8 digits entered, submitting form');
    ...
    if 'work_order' not in st.session_state:
        ...
        logger.debug("Initialized work_order in session state")
    ...
    logger.debug(f"Current work_order input: {work_order}")

    if submit_button or (work_order and len(work_order) == 8):
        logger.debug("Form submitted or 8-digit work order entered")
    ...
    for i in range(1, 10):
        if cols[i % 3].button(str(i), key=f"btn_{i}"):
            ...
            logger.debug(f"Button {i} pressed. New work_order: {st.session_state.work_order}")
            st.rerun()
    
    # Last row with 0, Clear, and Done
    if cols[0].button("Clear", key="btn_clear"):
        st.session_state.work_order = ''
        logger.debug("Clear button pressed. work_order cleared.")
        st.rerun()
    if cols[1].button("0", key="btn_0"):
        st.session_state.work_order += '0'
        logger.debug(f"Button 0 pressed. New work_order: {st.session_state.work_order}")
        st.rerun()
...
def validate_work_order(work_order):
    logger.debug("Entering validate_work_order function")
    work_order = work_order.strip()
    logger.debug(f"Validating work order: {work_order}")
    
    if len(work_order) != 8 or not work_order.isdigit():
        logger.warning("Invalid work order format")
    ...
    try:
        ...
        logger.debug(f"Query result: {order}")
        
        if order:
            logger.info(f"Valid Work Order: {work_order}")
            ...
            logger.debug("Set work_order_validated to True")
            # Remove the st.rerun() call from here
        else:
            logger.warning(f"Invalid Work Order Number: {work_order}")
